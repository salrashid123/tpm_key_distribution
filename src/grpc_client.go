...
func main() {
	...
	if err != nil {
		glog.Fatalf("Unable to find Default Credentials error=%v", err)
	}
	...
	glog.V(10).Infof("Acquired OIDC: %s", tok.AccessToken)
	rpcCreds, err := sal.NewIDTokenRPCCredential(ctx, idTokenSource)
	if err != nil {
		glog.Fatalf("Unable to get OID Token Credentials error=%v", err)
	}
...
func importKey(ar pb.ActivateCredentialResponse) (secret string, err error) {
...
func importRSAKey(ar pb.ActivateCredentialResponse) (err error) {
